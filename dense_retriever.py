...
def generate_question_vectors(
    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            """
            # logger.info("!!! batch_tensors %s %s", len(batch_tensors), batch_tensors[0].size())
...
class DenseRPCRetriever(DenseRetriever):
    ...
    def load_index(self, index_id):
        ...
        self.index_client.load_index(self.index_id,force_reload=False)
...
def validate_from_meta(
    ...
    match_stats = calculate_matches_from_meta(answers, result_ctx_ids, workers_num, match_type, use_title=True)
    top_k_hits = match_stats.top_k_hits
...
def main(cfg: DictConfig):
    ...
    if cfg.rpc_retriever_cfg_file:
        ...
        retriever = DenseRPCRetriever(encoder, cfg.batch_size, tensorizer, cfg.rpc_retriever_cfg_file, vector_size,
